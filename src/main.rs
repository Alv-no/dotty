...
fn main() {
    App::new()
        ...
        .add_systems(
            Update,
            (
                apply_gravity.before(apply_collision),
                apply_collision,
                death_dot,
                apply_x_movement,
            ),
        ...
        .add_systems(
            PostUpdate,
            (move_dot, handle_keyboard, camera_follow_dot.after(move_dot)),
        )
        .run();
}
...
fn setup(
    ...
    commands.spawn((
        ...
        YMovementState(Falling),
        XMovementState(Stopped),
        JumpingState(NoJump),
        Speed { x: 0., y: 0. },
        Dot {
            direction: Down,
...
fn init_map(map: &str, mut commands: Commands) {
    let mut x = 0;
    let mut y = 0;
...
fn apply_gravity(
    mut entities_with_speed: Query<
        (&mut Speed, &CollidedWithPlatform, &mut YMovementState),
        (With<Speed>, Without<Stationary>),
    >,
    time: Res<Time>,
) {
...
fn apply_collision(
    ...
    let x = for (
        mut transform,
        mut collided_with_platform,
        mut speed,
        mut movable_state,
        mut jumping_state,
    ) in movable_query.iter_mut()
    {
        let prev = collided_with_platform.0;
        let mut y = 0.;
        collided_with_platform.0 = platform_query.iter().any(|platform_transform| {
            let collision = speed.y < 0.
                && transform
                    .translation
                    .abs_diff_eq(platform_transform.translation, 10.);
            if collision {
...
fn move_dot(
    mut dot_query: Query<
        (&mut Transform, &mut Dot, &mut Speed, &CollidedWithPlatform),
        (Without<Platform>, With<Dot>),
    >,
    time: Res<Time>,
) {
...
fn handle_keyboard(
        ...
        dot_query.iter_mut()
    {
        ...
        if (keyboard_input.just_pressed(KeyCode::W)
            ...
            && (dot_state.0 == Standing || jumping_state.0 != DoubleJump)
        {
            dot.direction = Up;
            speed.y = 3.;
